...
export default class VideoRenderResponseHandler extends PubSubResponseHandler {
  ...
  private async deleteExistingDuplicateLinkTypes(storageLinkCreateInputs: VideoUploadStorageLinkCreateInput[], id: string) {
    const { storageLinks } = await prismaContext.query.videoUpload({ where: { id } }, ' { storageLinks { id path version bucket fileType } }');
    ...
    return prismaContext.mutation.deleteManyVideoUploadStorageLinks({ where: { id_in: toDelete } });
  }
