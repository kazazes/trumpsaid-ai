<template>
  <b-row class="mb-2">
    ...
    >
      <VideoPlayer
        id="master"
        ref="transcribePlayer"
        :poster="getPoster(videoUpload)"
        :sources="getSource(getMp4Web())"
        preload="auto"
        data-setup="{}"
        @playerTimeUpdate="playerTimeUpdated"
        @playerLoaded="playerMetadataUpdated"
      />
    </b-col>
    <b-col md="12">
      <div class="my-2">
        <b-btn
          v-b-modal.newSpeakerModal
          size="sm"
          class="mx-2"
          variant="primary"
        ...
        <div class="pull-right">
          <b-button-group>
            ...
            <b-btn
              size="sm"
              variant="warning"
              :disabled="editorDisabled || !draftSaved"
    ...
    <b-col md="12">
      <table class="table table-sm mx-2">
        ...
        <tbody>
          <tr
            v-for="(item, index) in editableTranscript"
            :key="index"
          >
            <td class="time">
              <a
                href=""
                @click.stop.prevent="seekTo(item.start)"
              >
              ...
              >
                <b-btn
                  ...
                  @click="timeButtonPressed('start', 0.1, item, index)"
                >+0.1</b-btn>
                <b-btn
                  :disabled="editorDisabled"
            ...
            <td class="time">
              <a
                href=""
                @click.stop.prevent="seekTo(item.end)"
              >
              ...
              >
                <b-btn
                  :disabled="editorDisabled"
                  variant="outline-primary"
                  @click="timeButtonPressed('end', 0.1, item, index)"
                >+0.1</b-btn>
                <b-btn
                  :disabled="editorDisabled"
                  variant="outline-info"
            ...
            <td class="actionButtons">
              ...
              >
                <b-button
                  v-if="index < editableTranscript.length - 1"
                  :disabled="editorDisabled"
                  variant="outline-primary"
                  @click="mergeDownPressed(index, item)"
                ><i class="icon icon-arrow-down" />&nbsp; Merge</b-button>
                <b-button
                  variant="outline-secondary"
                  :pressed="(looping && loopIndex === index)"
                  @click="loopPressed(index, item)"
                ><i class="icon icon-refresh" /></b-button>
                <b-button
                  :disabled="editorDisabled"
                  variant="outline-secondary"
                  @click="addBlockPressed(index, item)"
                ><i class="icon icon-plus" /></b-button>
                <b-button
                  :disabled="editorDisabled"
                  variant="outline-danger"
                  @click="deleteBlockPressed(index, item)"
                ><i class="icon icon-trash" /></b-button>
              </b-button-group>
            </td>
          </tr>
        </tbody>
